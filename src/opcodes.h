...
private:
    ...
    void code0xxx(uint16_t opcode); // 00E0: clear, 00EE: return
    ...
    void code6XNN(uint16_t opcode); // 6XNN: Set VX = NN
    void code7XNN(uint16_t opcode); // 7XNN: VX += NN (no carry)
    void code8XYN(uint16_t opcode); // 8XY0-8XYE: ALU ops
    void code9XY0(uint16_t opcode); // 9XY0: Skip if VX != VY
    void codeANNN(uint16_t opcode); // ANNN: I = NNN
    void codeBNNN(uint16_t opcode); // BNNN: Jump to NNN + V0
    void codeCXNN(uint16_t opcode); // CXNN: VX = rand() & NN
    void codeDXYN(uint16_t opcode); // DXYN: Draw sprite
    void codeEXNN(uint16_t opcode); // EX9E/EXA1: Key input
    void codeFXNN(uint16_t opcode); // FX07-FX65: Misc
};
